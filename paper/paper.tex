...
let $P_{ij} := x_i \cdot y_j$ for $i, j \in \{0, 1\}$. Define the
\emph{candidate set}
\[
\Cand(I, J) := \bigcup_{i,j \in \{0,1\}} \V(x_i \cdot y_j)
\;\subseteq\; \Rext,
\]
where the \emph{value map} $\V$ is defined symmetrically on each
ordered pair of factors:
\[
\V(p) := \begin{cases}
\{p\}, & p \in \Rext \text{ (a defined product)}, \\
...
The product is then
\[
I \cdot J := \inI{\min \Cand(I, J),\ \max \Cand(I, J)}.
\]
\end{definition}

Multiplication of finite reals with $\pm\infty$ commutes in $\Rext$,
so the only ordered pairs requiring symmetric extension are those
involving the indeterminate factor $0$. The endpoint sets
$\{0, \infty\}$ and $\{-\infty, 0\}$ returned by $\V$ on
indeterminate corner products are exactly the endpoints of the
foundation intervals $\Omega = \inI{0, \infty}$ and
$-\Omega = \inI{-\infty, 0}$ of Section~\ref{sec:foundation-rules};
taking $\min$ and $\max$ over $\Cand$ recovers those intervals.

In particular, if no $P_{ij}$ is an indeterminate form, the
definition reduces to the classical interval-arithmetic formula
\[
...
\end{cases}
\]
The four $r = 0$ subcases follow from $\V$ applied to the candidate
products $0 \cdot x_0$ and $0 \cdot x_1$. For \emph{infinite} scalars
$r \in \{-\infty, +\infty\}$ (which may produce indeterminate corner
products $r \cdot 0$ when $0 \in [x_0, x_1]$), no closed-form
...
\;\subseteq\; \Rext,
\]
where the additive value map
$\V_{+}: \Rext \cup \{\infty + (-\infty),\ (-\infty) + \infty\}
\to \mathcal{P}(\Rext)$ is
\[
\V_{+}(s) := \begin{cases}
\{s\}, & s \in \Rext \text{ (a defined sum)}, \\
\{-\infty, \infty\}, & s \in \{\infty + (-\infty),\ (-\infty) + \infty\}.
\end{cases}
\]
Then
\[
I + J := \inI{\min \Cand_{+}(I, J),\ \max \Cand_{+}(I, J)}.
\]
\end{definition}
...
With the analogous value map $\V_{-}$ that sends $\infty - \infty$
and $(-\infty) - (-\infty)$ to $\{-\infty, \infty\}$ and every defined
difference to a singleton,
\[
...
\[
I^{E} := \inI{\inf \mathcal{S}(I, E),\ \sup \mathcal{S}(I, E)},
\]
where
\[
...
\;\subseteq\; \Rext,
\]
and $\Vhat$ extends the real-valued map by handling the three
indeterminate-form points:
\[
\Vhat(b^{e}) := \begin{cases}
...
implementation handles many---but not necessarily all---non-admissible
inputs by NaN propagation and should not be relied upon to reject
every non-admissible input outside the cases exercised by the unit
tests.
...
$0 \in I$ (giving the corner $0^0$) or if $\pm\infty \in I$ (giving
the corners $(\pm\infty)^{0}$)---then $\Vhat$ contributes
...
the extended-base indeterminate point $(\pm\infty)^{0}$ does not
...
with $x_0, x_1, y_0, y_1 \in \Rext$. Each of the four products
$P_{ij} := x_i \cdot y_j$ for $i, j \in \{0, 1\}$ is either:
\begin{enumerate}[label=(\alph*),leftmargin=2em]
  \item a defined element of $\Rext$, or
  \item one of the four indeterminate forms $0 \cdot (\pm\infty)$ or
    $(\pm\infty) \cdot 0$, treated symmetrically by the value map
    $\V$ of Definition~\ref{def:multiplication} (Rules I and II for
    $0 \cdot \infty$ and $0 \cdot (-\infty)$, with the reversed forms
    $\infty \cdot 0$ and $(-\infty) \cdot 0$ assigned the same finite
    endpoint sets $\{0, \infty\}$ and $\{-\infty, 0\}$, respectively).
\end{enumerate}
In both cases the value map $\V(P_{ij})$ is a non-empty finite subset
of $\Rext$. Hence the candidate set
$\Cand(I, J) = \bigcup_{i,j} \V(P_{ij})$ is a non-empty finite subset
of $\Rext$, and its minimum and maximum are well-defined. Therefore
$I \cdot J = \inI{\min \Cand,\ \max \Cand} \in \Iset$, establishing
closure.
\end{proof}

\begin{proof}[Proof of identity]
For $I = \inI{x_0, x_1}$ with $x_0, x_1 \in \Rext$, the four endpoint
products in $\inI{1, 1} \cdot I$ are $1 \cdot x_0$ and $1 \cdot x_1$.
Since one factor is the finite nonzero scalar $1$, no indeterminate
product of the form $0 \cdot (\pm\infty)$ or $(\pm\infty) \cdot 0$
arises, and each endpoint product is defined in $\Rext$ with
$1 \cdot x_i = x_i$. Hence
$\Cand(\inI{1,1}, I) = \{x_0, x_1\}$ and
$\inI{1, 1} \cdot I = \inI{x_0, x_1} = I$. The argument is symmetric
in the operands.
\end{proof}
...
\begin{itemize}[leftmargin=2em]
  \item Ingeborg Kettern, for analysis at the Fachhochschule
    Furtwangen
  \item Prof.\ Dr.\ Peter Fleischer, for algebra at the
    Fachhochschule Furtwangen
  \item Eric Lengyel, for insightful questions during the more
    recent development of this work
  \item AI assistants, for review, critique, and editorial support
    during the preparation of the manuscript
\end{itemize}

\paragraph{Personal acknowledgments.} The author gratefully
acknowledges his parents, Monika Friedel Nopper and Ernst Christian
Nopper; his wife, Iris Karoline Nopper; his family and friends; and
