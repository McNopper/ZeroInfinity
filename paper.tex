...
interval numbers as a notational shorthand. The rules adopt a
\emph{directional} (one-sided) convention that refines the two-sided
assignment $[-\infty, \infty]$ of containment-set (cset) arithmetic
and is the precise choice under which the sign-reflection identity
$\Omega = -1 \cdot (-\Omega)$ carries algebraic content.
...
\begin{enumerate}[leftmargin=2em]
  \item A formal definition of \textbf{interval numbers} as a
    ...
    founded on a \emph{directional} rule
    $0 \cdot \infty = \inI{0, \infty}$ that strictly refines the
    two-sided assignment $[-\infty, \infty]$ of containment-set
    arithmetic~\cite{Walster1998,PryceCorliss2006}
...
Closest to the present work is \emph{containment-set (cset)
arithmetic}, introduced by Walster~\cite{Walster1998} and developed
by Pryce and Corliss~\cite{PryceCorliss2006}, which evaluates
expressions over the extended reals by assigning to each operation at
each point the set of \emph{all} attainable limits. Indeterminate
forms thereby receive set values: the cset of $x \cdot y$ at
$(0, +\infty)$ is the full extended line $[-\infty, +\infty]$,
because two-sided sequences $x_n \to 0$ realize every extended-real
limit. The present framework departs from cset arithmetic in three
substantive respects.
\begin{enumerate}[label=(\roman*),leftmargin=2.5em]
  \item \emph{Directional foundation rule.} Rule~I assigns
    $0 \cdot \infty$ the half-line $\inI{0, \infty}$, obtained by
    restricting the vanishing factor to one-sided approach
    (Section~\ref{sec:foundation-rules})---a strictly tighter
    interval than the cset $[-\infty, +\infty]$, and the choice that
    validates the motivating identity
    $\Omega = -1 \cdot (-\Omega)$
    (Section~\ref{sec:algebraic-consistency}), which collapses to a
    tautology under the two-sided assignment.
  \item \emph{Algebraic characterization.} Cset arithmetic was
    designed for rigorous numerical enclosure; the algebraic
    structure it induces on intervals was not analysed. Here the
    multiplicative structure is characterized exactly: closure and a
    two-sided identity hold (Theorem~\ref{thm:closure}) while
    associativity fails by explicit counterexample
    (Proposition~\ref{prop:non-assoc}), locating
    $(\Iset, \cdot, \inI{1,1})$ precisely at unital magma.
  \item \emph{Structural results.} Inclusion monotonicity is proved
    for all basic operations together with its exact obstruction---the
    single degenerate divisor $\inI{0,0}$
    (Proposition~\ref{prop:monotone},
    Remark~\ref{rem:recip-nonmono})---a statement that has no
    counterpart in the cset literature.
\end{enumerate}
...
are first-class algebraic objects. The individual ingredients---
extended-real endpoints, set-valued treatment of indeterminate
points, hull-based closure---are each present in the works cited
above, most completely in cset
arithmetic~\cite{Walster1998,PryceCorliss2006}. What is new here is,
first, the \emph{directional} foundation rule
$0 \cdot \infty = \inI{0, \infty}$, which refines the two-sided cset
assignment $[-\infty, +\infty]$ and is the precise choice under which
the sign-reflection identity $\Omega = -1 \cdot (-\Omega)$ carries
algebraic content; and second, the resulting \emph{algebraic-structure
analysis}---the unital-magma theorem, the non-associativity
counterexample, the embedding of $\Rext$, and the inclusion-monotonicity
result with its exact obstruction---none of which is developed in the
enclosure-oriented cset literature. To the author's knowledge, this
directional convention and the algebraic characterization built on it
have not previously been treated as a self-contained framework.
...
$[-\infty, \infty]$---precisely the containment set assigned to
$0 \cdot \infty$ by cset
arithmetic~\cite{Walster1998,PryceCorliss2006}---and the algebraic
identity that motivates the framework would collapse: with
$0 \cdot \infty = 0 \cdot (-\infty) = \inI{-\infty, \infty}$, the
identity $\Omega = -1 \cdot (-\Omega)$ degenerates to the tautology
$\inI{-\infty, \infty} = \inI{-\infty, \infty}$ and distinguishes
nothing. The directional refinement is therefore the point of
departure from cset arithmetic, and the tighter foundation intervals
it yields are what give the algebraic analysis of
Section~\ref{sec:algebraic-structure} its content.
...
\newblock Technical report, Sun Microsystems Laboratories, 1998.

\bibitem{PryceCorliss2006}
J.~D. Pryce and G.~F. Corliss.
\newblock Interval arithmetic with containment sets.
\newblock \emph{Computing} 78(3), 251--276, 2006.
\newblock \href{https://doi.org/10.1007/s00607-006-0180-4}{doi:10.1007/s00607-006-0180-4}.
