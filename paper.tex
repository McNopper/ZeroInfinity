...
For any nonempty finite subset $S \subseteq \Rext$, define the
\emph{interval hull}
\[
\mathrm{Hull}(S) \;:=\; [\,\min S,\ \max S\,].
\]
Throughout the paper, $\min$ and $\max$ are taken in the extended
order on $\Rext$, so $\min S$ and $\max S$ exist and lie in $\Rext$
for any nonempty finite $S$.

\begin{proposition}[Well-definedness of the hull]\label{prop:hull}
For every nonempty finite $S \subseteq \Rext$, $\mathrm{Hull}(S)$ is
an interval number in the sense of
Definition~\ref{def:interval-number}; equivalently,
$\min S, \max S \in \Rext$ and $\min S \le \max S$.
\end{proposition}

\begin{proof}
$\Rext$ is totally ordered, so any nonempty finite $S \subseteq \Rext$
has a least and a greatest element in $\Rext$, with
$\min S \le \max S$ by definition. Hence
$\mathrm{Hull}(S) = [\min S, \max S]$ satisfies
Definition~\ref{def:interval-number}.
\end{proof}
...
$\mathrm{Hull}(L^{+}) = [\min L^{+}, \max L^{+}] = [0, +\infty]$.
...
\;\subseteq\; \Rext,
\]
and the exponentiation value map $\Vhat : \Rext \times \Rext \to
\mathcal{P}_{\mathrm{fin}}(\Rext) \setminus \{\emptyset\}$ extends
real-valued exponentiation by handling the indeterminate-form points:
\[
\Vhat(b, e) \;:=\;
\begin{cases}
...
\end{cases}
\]
Well-definedness of $\mathcal{S}(I, E)$ as a nonempty subset of
$\Rext$, and of $I^{E}$ as an interval number, requires that
$\mathcal{S}(I, E)$ have a finite-element minimum and maximum in
$\Rext$ on the admissible domain; this follows because the
real-valued continuous map $(x, y) \mapsto x^{y}$ is monotone in each
argument on each of the rectangles obtained by splitting $I \times E$
along $x = 1$ and $y = 0$, so its image is a closed interval, and the
...
\]
The same holds for the unary operations $|\cdot|$ and $1/\,\cdot\,$
(Definitions~\ref{def:abs} and~\ref{def:reciprocal}), and for
exponentiation $I^E$ on pairs $(I, E), (I', E')$ that both lie in the
admissible domain of Definition~\ref{def:power}.
\end{proposition}

\begin{proof}
Each operation is defined as the hull of a candidate (image) set
constructed from the endpoints (or points) of its operands. If
$I \subseteq I'$, the endpoints of $I$ lie in $I'$ and hence among
the points considered by the corresponding image set for $I'$;
similarly for $J \subseteq J'$. Therefore the candidate set for
$I \circ J$ is a subset of that for $I' \circ J'$, and
$\mathrm{Hull}(S) \subseteq \mathrm{Hull}(S')$ whenever
$S \subseteq S'$, so
$I \circ J \subseteq I' \circ J'$. The same argument applies to
$|\cdot|$ (whose definition is by cases on the order of endpoints,
each case being inclusion-monotone) and to $1/\,\cdot\,$ (by
inspection of the five-case definition: each case is monotone in the
endpoints, and inclusion across case boundaries broadens the result).
For exponentiation, the image set $\mathcal{S}(I, E)$ is by
construction monotone in $(I, E)$ under set inclusion.
\end{proof}
